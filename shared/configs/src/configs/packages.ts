...
export default <Record<PACKAGE_ENUM, IPackagesConfig>>{
	...
	[PACKAGE_ENUM.DEMO]: {
		...
		microConfig: {
			...
			entry: process.env.NODE_ENV === 'production' ? `/${PACKAGE_ENUM.DEMO}/index.html` : `//localhost:${PORT_ENUM.DEMO}`,
	...
	[PACKAGE_ENUM.BLOG_ADMIN]: {
		...
		microConfig: {
			...
			entry:
				process.env.NODE_ENV === 'production' ? `/${PACKAGE_ENUM.BLOG_ADMIN}/index.html` : `//localhost:${PORT_ENUM.BLOG_ADMIN}`,
