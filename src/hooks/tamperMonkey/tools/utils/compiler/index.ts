...
//path.resolve(path.dirname(__dirname)) + './src/hooks/tamperMonkey/tools/src/index.vue'
try {
	const file = fs.readFileSync(path.resolve(__dirname, '../../src/index.vue'), {
		encoding: 'utf-8',
	});

	const codeList = [];
	const id = Date.now().toString().slice(0, 5); // 这个 id 是 scopeId，用于 css scope，保证唯一即可
	const scopeId = `data-harver-${id}`;
