...
import * as glslChunk from '@demo/glslChunk';
export class _Math {
	constructor() {}
	...
	center(
		params: THREE.Group | THREE.Mesh | [number, number][] | THREE.Vector2[]
	): [THREE.Vector3, THREE.Box3] | [THREE.Vector3, THREE.Vector3] {
		if (params instanceof THREE.Object3D) {
			const box = new THREE.Box3();
			box.expandByObject(params);
			return [box.max.clone().add(box.min).multiplyScalar(-0.5), box];
		} else {
			if (!Array.isArray(params)) return params;
			let geometry,
				isV2 = params[0] instanceof THREE.Vector2;
	...
	createParticleBuffer({
			...
			colorArray.push(
				color
					? typeof color === 'function'
						? color(i, len)
						: color
					: [1 - i / len, THREE.MathUtils.randFloat(0.4, 0.6), i / len, 1]
			);
			nextColorArray.push(
				nextColor
		...
		isRandomProcess && processArray.sort(() => Math.random() - 0.5);
